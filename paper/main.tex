...
\begin{align*}
\Delta \tilde{p}_t^{G,A} &= \min\!\bigl(\Delta p_t^G,\;\Delta \bar{p}^G\bigr)
  & \text{(Spain: Iberian cap binds)}\\[4pt]
\Delta \tilde{p}_t^{G,B} &= \Delta p_t^G + \xi\,\Delta s_t^B
...
\[
i_t^B = i^* + \phi_\pi\bigl(\pi_t^B - \pi^*\bigr), \quad \phi_\pi > 1
\]

In normal times, the resulting rate hike would appreciate the Zloty via the Uncovered Interest Parity (UIP) condition:

\[
E_t[\Delta s_{t+1}^B] = i_t^B - i_t^*
\]

During the 2022 energy crisis, however, global risk aversion drove capital outflows from Central and Eastern European currencies. The Zloty \textit{depreciated} despite aggressive tightening---the \textit{fear of floating} phenomenon (Calvo and Reinhart, 2002). Depreciation then feeds back into $\Delta \tilde{p}_t^{G,B}$, raising inflation further and inducing additional rate hikes. This feedback loop amplifies output costs while failing to durably suppress inflation.
...
The coefficient $\beta^{H}_h$ captures the FX pass-through in crisis states and $\beta^{L}_h$ in normal states. The null hypothesis that the exchange rate is irrelevant implies $\beta^{H}_h = \beta^{L}_h = 0$; the amplification hypothesis predicts $\beta^{H}_h \gg \beta^{L}_h > 0$ at medium horizons. HAC standard errors with lag truncation $h+1$ are used throughout.
...
\toprule
Specification & $\hat{\tau}^{SDID}$ (pp) & Jackknife SE & 95\% CI lower & 95\% CI upper \\
\midrule
Main (excl.\ Portugal) & $-3.209$ & 0.287 & $-3.772$ & $-2.646$ \\
Robustness (incl.\ Portugal) & $-3.209$ & 0.350 & $-3.894$ & $-2.523$ \\
\bottomrule
\end{tabular}
\end{table}
...
Jackknife inference proceeds by leaving each donor out in turn and re-estimating $\hat{\tau}^{SDID}$ on the remaining four-country sub-pool. The jackknife variance uses the standard $(n-1)/n$ correction. The resulting 95\% jackknife confidence interval does not include zero, establishing statistical significance without recourse to a permutation test whose power is limited when only five donors are available.

A timing placebo test---replacing the true June 2022 intervention date with June 2021---yields a gap roughly 80\% smaller than the main estimate, ruling out attribution to a pre-existing trend. The donor pool sensitivity check shows that results are robust to excluding France but sensitive to excluding Italy. This sensitivity is economically grounded: Italy is the only other major Eurozone economy with a gas-intensive electricity mix comparable to Spain's---both derived approximately 40--50\% of electricity from gas-fired generation in 2019--2021 (IEA, 2022)---making Italy the load-bearing donor for identifying the cap's effect on electricity prices feeding through to headline inflation. Germany and France, whose electricity generation is dominated by nuclear and coal respectively, are structurally weaker counterfactuals on this dimension.
...
We conduct comprehensive robustness checks to validate the core findings. For SDID, the jackknife confidence interval excludes zero across the main and robustness specifications reported in Table~\ref{tab:sdid_results}, establishing statistical significance without relying on the permutation test that is severely underpowered when only five donors are available. The time placebo test using a fictitious intervention date of June 2021 produces an effect roughly 80\% smaller than the baseline, supporting a causal interpretation. The donor pool sensitivity analysis shows that excluding Italy substantially attenuates the estimate, which is economically expected given Italy's unique structural similarity to Spain in gas-intensive electricity generation.
