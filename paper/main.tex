...
\[
i_t^B = \bar{i} + \phi_\pi\bigl(\pi_t^B - \pi^*\bigr) + \phi_y\tilde{y}_t^B, \quad \phi_\pi > 1
\]

where $\phi_y \geq 0$ is the output gap coefficient and $\tilde{y}_t^B$ is the output gap. In normal times, the resulting rate hike would appreciate the Zloty via the Uncovered Interest Parity (UIP) condition:

\[
\mathbb{E}_t\bigl[s_{t+1}^B\bigr] - s_t^B = i_t^B - i_t^*
\]
